...
pub async fn run_migrations(pool: &SqlitePool) -> Result<(), sqlx::Error> {
    ...
    // Also remove the now-unused index on status
    ...
    if has_deprecated.unwrap_or(0) > 0 {
        ...
        // Step 1: Create new payments table without deprecated columns
        sqlx::query(
            "CREATE TABLE payments_new (
                ...
                recurrence_times_per INTEGER,
                recurrence_end_date TEXT,
                receiver_account_id INTEGER REFERENCES participants(id) ON DELETE SET NULL,
                recurrence_weekdays TEXT,
                recurrence_monthdays TEXT,
                recurrence_months TEXT,
                is_final INTEGER NOT NULL DEFAULT 1,
                affects_balance INTEGER NOT NULL DEFAULT 1,
                affects_payer_expectation INTEGER NOT NULL DEFAULT 0,
                affects_receiver_expectation INTEGER NOT NULL DEFAULT 0
            )",
        )
        .execute(pool)
        .await?;

        // Step 2: Copy data from old table to new (excluding deprecated columns)
        sqlx::query(
            "INSERT INTO payments_new (
                id, project_id, payer_id, amount, description, payment_date, created_at,
                ...
                recurrence_weekdays, recurrence_monthdays, recurrence_months,
                is_final, affects_balance, affects_payer_expectation, affects_receiver_expectation
            )
            SELECT
                id, project_id, payer_id, amount, description, payment_date, created_at,
                receipt_image, is_recurring, recurrence_type, recurrence_interval,
        ...
        sqlx::query("DROP TABLE payments").execute(pool).await?;

        // Step 4: Rename new table to original name
        sqlx::query("ALTER TABLE payments_new RENAME TO payments")
            .execute(pool)
            .await?;
        ...
        tracing::info!("Deprecated columns removed from payments table");
    }
