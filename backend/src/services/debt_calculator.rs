...
use crate::models::{AccountInterestRule, Payment};
...
pub struct PaymentOccurrence {
    ...
    // Synthetic occurrence generated by an account_interest_rule (payment_id is
    // -rule_id). Excluded from settlements; attributed to pool ownership
    // proportionally to each participant's ownership share at the credit date.
...
pub async fn calculate_debts_at_date(
    ...
    // Generate synthetic interest occurrences for pool accounts with interest
    // rules (Phase 1). They are excluded from settlements and attributed to
    // pool ownership proportionally — see generate_interest_occurrences.
    let interest_rules: Vec<AccountInterestRule> = sqlx::query_as(
        "SELECT r.* FROM account_interest_rules r
         JOIN participants p ON p.id = r.participant_id
         WHERE p.project_id = ? AND p.account_type = 'pool'",
    )
    .bind(project_id)
    .fetch_all(pool)
    .await?;

    if !interest_rules.is_empty() {
        // Chronological order is required for the balance walk
        all_occurrences.sort_by(|a, b| a.occurrence_date.cmp(&b.occurrence_date));

        let mut rules_by_pool: HashMap<i64, Vec<AccountInterestRule>> = HashMap::new();
        for rule in interest_rules {
            rules_by_pool
                .entry(rule.participant_id)
                .or_default()
                .push(rule);
        }
        for (interest_pool_id, rules) in rules_by_pool {
            let interest_occurrences = generate_interest_occurrences(
                interest_pool_id,
                &rules,
                &all_occurrences,
                &contribution_map,
            ...
            all_occurrences.extend(interest_occurrences);
        }
        all_occurrences.sort_by(|a, b| a.occurrence_date.cmp(&b.occurrence_date));
    }
    ...
    for occurrence in &all_occurrences {
        // Interest is a pool-only concept: never enters user-to-user settlements
        if occurrence.is_interest {
            continue;
        }
    ...
    for (pool_id, pool_name) in pool_account_list {
        ...
        for occurrence in &all_occurrences {
            // Synthetic interest: attribute proportionally to current ownership.
            // Handled before the generic paths (which rely on contribution rows
            // that synthetic occurrences don't have).
            if occurrence.is_interest {
                let credit_here = occurrence.receiver_account_id == Some(pool_id);
                let charge_here = occurrence.payer_id == Some(pool_id)
                    && occurrence.receiver_account_id.is_none();
                if credit_here || charge_here {
                    distribute_interest_to_ownership(&mut ownership_map, occurrence, credit_here);
                }
                continue;
            }
...
// ─── Interest on pool accounts (Phase 1, docs/PHASE1_ACCOUNTS.md) ───────────

/// Round to 4 decimal places — the project-wide fixed-point convention for
/// monetary amounts (same as contribution splitting).
fn round4(x: f64) -> f64 {
    (x * 10000.0).round() / 10000.0
}

/// Excel-compatible DAYS360, US/NASD method (the workbook's convention for
/// prorating the first annual interest credit).
fn days360(start: NaiveDate, end: NaiveDate) -> i64 {
    let (mut d1, m1, y1) = (
        start.day() as i64,
        start.month() as i64,
        start.year() as i64,
...
/// Signed effect of an occurrence on a pool's total balance. Mirrors the
/// ownership tracking rules exactly, so the balance interest is computed on
/// always equals the pool's displayed total_balance.
fn pool_balance_delta(
    ...
    if let Some(receiver_id) = occurrence.receiver_account_id {
        if let Some(payer_id) = occurrence.payer_id {
            if receiver_id == pool_id && payer_id != pool_id {
                return occurrence.amount; // deposit to pool
            }
            if payer_id == pool_id && receiver_id != pool_id {
                return -occurrence.amount; // withdrawal from pool
            }
        } else if receiver_id == pool_id {
            // External inflow to pool: contributors' shares increase ownership
            return contribution_map
                .get(&occurrence.payment_id)
                .map(|contribs| {
                    contribs
                        .iter()
    ...
    } else if let Some(payer_id) = occurrence.payer_id {
        if payer_id == pool_id {
            // Pool pays external expense: contributors' consumption
            return -contribution_map
                .get(&occurrence.payment_id)
                .map(|contribs| {
                    contribs
                ...
                .unwrap_or(0.0);
        } else if let Some(contribs) = contribution_map.get(&occurrence.payment_id) {
            // User pays an expense the pool contributes to: payer's ownership grows
            if let Some((_, pool_amount)) = contribs.iter().find(|(pid, _)| *pid == pool_id) {
                return *pool_amount;
            }
        }
    }
...
/// Period boundary dates strictly after `after`, up to and including `until`.
/// Boundaries follow the workbook's calendar convention: monthly on the 1st,
/// semiannual on Jun 1 / Dec 1 (Canadian mortgage compounding), annual Jan 1.
fn period_boundaries(period: &str, after: NaiveDate, until: NaiveDate) -> Vec<NaiveDate> {
    let mut boundaries = Vec::new();
    let mut next = match period {
        "annual" => NaiveDate::from_ymd_opt(after.year() + 1, 1, 1),
        "semiannual" => {
            let candidates = [
                NaiveDate::from_ymd_opt(after.year(), 6, 1),
                NaiveDate::from_ymd_opt(after.year(), 12, 1),
                NaiveDate::from_ymd_opt(after.year() + 1, 6, 1),
            ];
            candidates.into_iter().flatten().find(|d| *d > after)
        }
        // monthly
        _ => NaiveDate::from_ymd_opt(after.year(), after.month(), 1)
            .and_then(|d| d.checked_add_months(Months::new(1))),
    };
    ...
    while let Some(date) = next {
        ...
        next = date.checked_add_months(Months::new(step));
    }
    boundaries
}

/// Generate synthetic interest occurrences for one pool account by walking its
/// balance chronologically and applying each rule at its period boundaries.
///
/// Conventions (matching the reference workbook — see docs/PHASE1_ACCOUNTS.md):
/// - A boundary credit uses the balance accumulated *strictly before* the
///   boundary date, including previously credited interest (compounding).
/// - Annual rules prorate their first credit with DAYS360(start, boundary)/360;
///   monthly and semiannual rules charge full periods from the first boundary.
/// - `only_when_negative` rules skip the rate part while the balance is >= 0.
/// - `monthly_fee` (if set) is charged at each boundary while balance is below
///   `fee_threshold` (no threshold = always), independent of the rate part.
/// - Amounts are rounded to 4 decimals; zero-amount ticks emit nothing.
/// - Nothing is charged before the pool has any balance activity.
///
/// Synthetic occurrences carry `payment_id = -rule.id` and `is_interest: true`.
pub fn generate_interest_occurrences(
    pool_id: i64,
    rules: &[AccountInterestRule],
    real_occurrences: &[PaymentOccurrence],
    contribution_map: &HashMap<i64, Vec<(i64, f64)>>,
    target: NaiveDate,
) -> Vec<PaymentOccurrence> {
    let mut deltas: Vec<(NaiveDate, f64)> = real_occurrences
        .iter()
        .filter(|o| o.affects_balance && !o.is_interest)
        .filter_map(|o| {
            let delta = pool_balance_delta(o, pool_id, contribution_map);
            if delta == 0.0 {
                None
            } else {
                parse_date(&o.occurrence_date).map(|d| (d, delta))
            }
        })
        .collect();
    deltas.sort_by_key(|(date, _)| *date);

    let mut sorted_rules: Vec<&AccountInterestRule> = rules.iter().collect();
    sorted_rules.sort_by(|a, b| a.start_date.cmp(&b.start_date));

    // (boundary_date, rule, fraction of a period for the rate part)
    let mut ticks: Vec<(NaiveDate, &AccountInterestRule, f64)> = Vec::new();
    for rule in &sorted_rules {
        let Some(start) = parse_date(&rule.start_date) else {
            continue;
        };
        let until = rule
            .end_date
            .as_ref()
            .and_then(|d| parse_date(d))
            .map_or(target, |end| end.min(target));
        for (i, boundary) in period_boundaries(&rule.period, start, until)
            .into_iter()
            .enumerate()
        {
            let fraction = if rule.period == "annual" && i == 0 {
                days360(start, boundary).max(0) as f64 / 360.0
            } else {
                1.0
            };
            ticks.push((boundary, rule, fraction));
        }
    }
    ticks.sort_by_key(|(date, _, _)| *date);
    ...
    for (boundary, rule, fraction) in ticks {
        while delta_idx < deltas.len() && deltas[delta_idx].0 < boundary {
            balance += deltas[delta_idx].1;
            has_activity = true;
            delta_idx += 1;
        }
        if !has_activity {
        ...
        let periods_per_year = match rule.period.as_str() {
            "semiannual" => 2.0,
            "annual" => 1.0,
            _ => 12.0,
        };
        let mut amount = 0.0;
        if !(rule.only_when_negative && balance >= 0.0) {
            amount += balance * rule.annual_rate * fraction / periods_per_year;
        }
        if let Some(fee) = rule.monthly_fee {
            if rule.fee_threshold.is_none_or(|t| balance < t) {
                amount -= fee;
            }
        }
        let amount = round4(amount);
        if amount == 0.0 {
            continue;
        }
        balance += amount;

        let (payer_id, receiver_account_id, occurrence_amount) = if amount > 0.0 {
            (None, Some(pool_id), amount)
        } else {
            (Some(pool_id), None, -amount)
        };
        result.push(PaymentOccurrence {
            payment_id: -rule.id,
            description: "Interest".to_string(),
            amount: occurrence_amount,
            occurrence_date: boundary.format("%Y-%m-%d").to_string(),
            payer_id,
            is_recurring: false,
            receiver_account_id,
            is_final: true,
...
/// Attribute an interest occurrence to pool owners proportionally to their
/// ownership share at that point in time (equal split if the pool nets to
/// zero). Shares are rounded to 4 decimals with the residual cent-fraction
/// assigned to the largest holder so the pieces always sum to the amount.
fn distribute_interest_to_ownership(
    ownership_map: &mut HashMap<i64, OwnershipMapEntry>,
    occurrence: &PaymentOccurrence,
    is_credit: bool,
) {
    ...
    let total: f64 = ownership_map
        .values()
        .map(|(contributed, consumed, _, _)| contributed - consumed)
        .sum();

    let mut shares: Vec<(i64, f64)> = ownership_map
        .iter()
        .map(|(id, (contributed, consumed, _, _))| {
            let raw = if total.abs() < 1e-9 {
                occurrence.amount / ownership_map.len() as f64
            } else {
                occurrence.amount * (contributed - consumed) / total
            };
            (*id, round4(raw))
        })
        .collect();
    ...
    // Assign the rounding residual to the largest |share| (deterministic).
    let residual = round4(occurrence.amount - shares.iter().map(|(_, s)| s).sum::<f64>());
    if residual != 0.0 {
        if let Some(entry) = shares.iter_mut().max_by(|a, b| {
            a.1.abs()
                .partial_cmp(&b.1.abs())
                .unwrap_or(std::cmp::Ordering::Equal)
                .then(b.0.cmp(&a.0))
        }) {
            entry.1 = round4(entry.1 + residual);
        }
    }
    ...
    for (participant_id, share) in shares {
        ...
        let breakdown = PairwisePaymentBreakdown {
            payment_id: occurrence.payment_id,
            description: occurrence.description.clone(),
            occurrence_date: occurrence.occurrence_date.clone(),
            amount: share,
        ...
        if is_credit {
            entry.0 += share;
            entry.2.push(breakdown);
        } else {
            entry.1 += share;
            entry.3.push(breakdown);
        }
    }
}
...
mod tests {
    ...
    // ─── Interest engine tests (Phase 1, docs/PHASE1_ACCOUNTS.md) ───────────
    ...
    #[test]
    fn test_days360_matches_excel() {
        let d = |y, m, dd| NaiveDate::from_ymd_opt(y, m, dd).unwrap();
        // Values verified against Excel DAYS360 (US/NASD method)
        assert_eq!(days360(d(2026, 7, 5), d(2027, 1, 1)), 176);
        assert_eq!(days360(d(2026, 1, 15), d(2027, 1, 15)), 360);
        assert_eq!(days360(d(2026, 1, 31), d(2026, 2, 28)), 28);
        assert_eq!(days360(d(2026, 1, 30), d(2026, 3, 31)), 60);
        assert_eq!(days360(d(2026, 3, 1), d(2026, 3, 1)), 0);
    }

    #[test]
    fn test_interest_monthly_simple_compounds() {
        // $1000 deposited Jan 15, 12% monthly rule from Jan 10.
        // Feb 1: 1000 * 0.12/12 = 10.0000; Mar 1: 1010 * 0.01 = 10.1000
        let pool_id = 99;
        ...
        let rules = vec![make_rule(1, "2026-01-10", None, 0.12, "monthly")];
        let target = NaiveDate::from_ymd_opt(2026, 3, 15).unwrap();

        let interest =
            generate_interest_occurrences(pool_id, &rules, &occurrences, &HashMap::new(), target);

        assert_eq!(interest.len(), 2);
        assert_eq!(interest[0].occurrence_date, "2026-02-01");
        assert_close(interest[0].amount, 10.0);
        assert_eq!(interest[0].receiver_account_id, Some(pool_id));
        assert_eq!(interest[0].payer_id, None);
        assert!(interest[0].is_interest);
        assert_eq!(interest[0].payment_id, -1); // -rule.id sentinel
        assert_eq!(interest[1].occurrence_date, "2026-03-01");
        assert_close(interest[1].amount, 10.1); // compounds on prior credit
    }

    #[test]
    fn test_interest_only_when_negative_skips_positive_balance() {
        let pool_id = 99;
        let occurrences = vec![make_occurrence(
            1,
            "2026-01-15",
        ...
        let mut rule = make_rule(1, "2026-01-10", None, 0.06, "monthly");
        rule.only_when_negative = true;
        let target = NaiveDate::from_ymd_opt(2026, 4, 15).unwrap();

        let interest =
            generate_interest_occurrences(pool_id, &[rule], &occurrences, &HashMap::new(), target);

        assert!(interest.is_empty());
    }

    #[test]
    fn test_interest_credit_line_charges_negative_balance() {
        // Pool → user withdrawal of $1000 leaves the pool at -1000.
        // 6% only-when-negative: -1000 * 0.06/12 = -5 → charge of 5, pool pays.
        let pool_id = 99;
        let occurrences = vec![make_occurrence(
            1,
            "2026-01-15",
        ...
        assert_eq!(interest[0].occurrence_date, "2026-02-01");
        assert_close(interest[0].amount, 5.0);
        assert_eq!(interest[0].payer_id, Some(pool_id)); // charge: pool pays
        assert_eq!(interest[0].receiver_account_id, None);
    }

    #[test]
    fn test_interest_monthly_fee_below_threshold() {
        // Zero rate, 7.95 fee while balance < 5000 (the workbook's checking fee)
        let pool_id = 99;
        let occurrences = vec![make_occurrence(
            1,
            "2026-01-15",
        ...
        let mut rule = make_rule(1, "2026-01-10", None, 0.0, "monthly");
        rule.monthly_fee = Some(7.95);
        rule.fee_threshold = Some(5000.0);
        let target = NaiveDate::from_ymd_opt(2026, 3, 15).unwrap();
        ...
        assert_eq!(interest.len(), 2);
        assert_close(interest[0].amount, 7.95);
        assert_eq!(interest[0].payer_id, Some(pool_id));

        // Above the threshold: no fee
        let big = vec![make_occurrence(
            1,
            "2026-01-15",
            9000.0,
            Some(1),
        ...
        let mut rule2 = make_rule(1, "2026-01-10", None, 0.0, "monthly");
        rule2.monthly_fee = Some(7.95);
        rule2.fee_threshold = Some(5000.0);
        let none = generate_interest_occurrences(pool_id, &[rule2], &big, &HashMap::new(), target);
        assert!(none.is_empty());
    }

    #[test]
    fn test_interest_annual_prorated_first_year() {
        // $10000 on 2026-07-05, 3.6% annual: first credit at Jan 1 prorated by
        // DAYS360(2026-07-05, 2027-01-01)/360 = 176/360.
        // 10000 * 0.036 * 176/360 = 176.0000
        let pool_id = 99;
        let occurrences = vec![make_occurrence(
            1,
            "2026-07-05",
        ...
        let rules = vec![make_rule(1, "2026-07-05", None, 0.036, "annual")];
        let target = NaiveDate::from_ymd_opt(2027, 1, 2).unwrap();

        let interest =
            generate_interest_occurrences(pool_id, &rules, &occurrences, &HashMap::new(), target);

        assert_eq!(interest.len(), 1);
        assert_eq!(interest[0].occurrence_date, "2027-01-01");
        assert_close(interest[0].amount, 176.0);
    }

    #[test]
    fn test_interest_semiannual_boundaries() {
        // Canadian mortgage convention: credits on Jun 1 and Dec 1.
        let pool_id = 99;
        let occurrences = vec![make_occurrence(
            1,
            "2026-01-20",
        ...
        let rules = vec![make_rule(1, "2026-01-15", None, 0.02, "semiannual")];
        let target = NaiveDate::from_ymd_opt(2026, 12, 15).unwrap();

        let interest =
            generate_interest_occurrences(pool_id, &rules, &occurrences, &HashMap::new(), target);

        assert_eq!(interest.len(), 2);
        assert_eq!(interest[0].occurrence_date, "2026-06-01");
        assert_close(interest[0].amount, 100.0); // 10000 * 0.02/2
        assert_eq!(interest[1].occurrence_date, "2026-12-01");
        assert_close(interest[1].amount, 101.0); // 10100 * 0.01, compounded
    }

    #[test]
    fn test_interest_rate_renewal_two_rules() {
        // Renewal on Mar 1: 12% rule ends exactly there, 6% rule starts there.
        // Feb 1 (12%): 10.0; Mar 1 belongs to the OLD rule (start < b <= end): 10.1;
        // the new rule's first boundary is strictly after its start: Apr 1 (6%).
        let pool_id = 99;
        let occurrences = vec![make_occurrence(
            1,
            "2026-01-15",
        ...
        let rules = vec![
            make_rule(1, "2026-01-10", Some("2026-03-01"), 0.12, "monthly"),
            make_rule(2, "2026-03-01", None, 0.06, "monthly"),
        ];
        let target = NaiveDate::from_ymd_opt(2026, 4, 15).unwrap();
        ...
        assert_eq!(
            (interest[0].occurrence_date.as_str(), interest[0].payment_id),
            ("2026-02-01", -1)
        );
        assert_close(interest[0].amount, 10.0);
        assert_eq!(
            (interest[1].occurrence_date.as_str(), interest[1].payment_id),
            ("2026-03-01", -1)
        );
        assert_close(interest[1].amount, 10.1);
        assert_eq!(
            (interest[2].occurrence_date.as_str(), interest[2].payment_id),
            ("2026-04-01", -2)
        );
        assert_close(interest[2].amount, 5.1005); // 1020.1 * 0.06/12
    }

    #[test]
    fn test_interest_waits_for_first_activity() {
        // Fee rule active since Jan 1, but the pool sees no money until Mar 15:
        // nothing is charged at Feb 1 or Mar 1 boundaries.
        let pool_id = 99;
        let occurrences = vec![make_occurrence(
            1,
            "2026-03-15",
        ...
        let mut rule = make_rule(1, "2026-01-01", None, 0.0, "monthly");
        rule.monthly_fee = Some(5.0);
        let target = NaiveDate::from_ymd_opt(2026, 5, 15).unwrap();

        let interest =
            generate_interest_occurrences(pool_id, &[rule], &occurrences, &HashMap::new(), target);

        assert_eq!(interest.len(), 2);
        assert_eq!(interest[0].occurrence_date, "2026-04-01");
        assert_eq!(interest[1].occurrence_date, "2026-05-01");
    }

    #[test]
    fn test_interest_uses_pool_paid_expense_contributions() {
        // Pool pays a $300 external expense split across users: pool balance
        // drops by the contributors' shares, and the next credit compounds on
        // the reduced balance.
        let pool_id = 99;
        let occurrences = vec![
            make_occurrence(1, "2026-01-10", 1000.0, Some(1), Some(pool_id)),
            make_occurrence(2, "2026-01-20", 300.0, Some(pool_id), None),
        ];
        let mut contribution_map: HashMap<i64, Vec<(i64, f64)>> = HashMap::new();
        contribution_map.insert(2, vec![(1, 150.0), (2, 150.0)]);
        let rules = vec![make_rule(1, "2026-01-01", None, 0.12, "monthly")];
        let target = NaiveDate::from_ymd_opt(2026, 2, 15).unwrap();

        let interest =
            generate_interest_occurrences(pool_id, &rules, &occurrences, &contribution_map, target);

        assert_eq!(interest.len(), 1);
        assert_close(interest[0].amount, 7.0); // (1000 - 300) * 0.01
    }

    #[test]
    fn test_distribute_interest_proportional_to_ownership() {
        let mut ownership_map: HashMap<i64, OwnershipMapEntry> = HashMap::new();
        ownership_map.insert(1, (750.0, 0.0, Vec::new(), Vec::new()));
        ownership_map.insert(2, (250.0, 0.0, Vec::new(), Vec::new()));

        let mut occurrence = make_occurrence(-1, "2026-02-01", 10.0, None, Some(99));
        occurrence.is_interest = true;

        distribute_interest_to_ownership(&mut ownership_map, &occurrence, true);

        let a = &ownership_map[&1];
        let b = &ownership_map[&2];
        assert_close(a.0, 757.5); // 750 + 10 * 0.75
        assert_close(b.0, 252.5); // 250 + 10 * 0.25
        assert_eq!(a.2.len(), 1); // breakdown entry appended
        assert_close(a.2[0].amount, 7.5);
    }

    #[test]
    fn test_distribute_interest_residual_sums_exactly() {
        // Three equal owners of $10: shares round to 3.3333 each; the residual
        // 0.0001 lands on one owner so the total credited is exactly 10.
        let mut ownership_map: HashMap<i64, OwnershipMapEntry> = HashMap::new();
        for id in 1..=3 {
            ownership_map.insert(id, (100.0, 0.0, Vec::new(), Vec::new()));
        }
        let mut occurrence = make_occurrence(-1, "2026-02-01", 10.0, None, Some(99));
        occurrence.is_interest = true;
        ...
        let credited: f64 = ownership_map.values().map(|(c, _, _, _)| c - 100.0).sum();
        assert_close(credited, 10.0);
    }

    #[test]
    fn test_distribute_interest_charge_goes_to_consumed() {
        let mut ownership_map: HashMap<i64, OwnershipMapEntry> = HashMap::new();
        ownership_map.insert(1, (1000.0, 0.0, Vec::new(), Vec::new()));

        let mut occurrence = make_occurrence(-1, "2026-02-01", 7.95, Some(99), None);
        occurrence.is_interest = true;

        distribute_interest_to_ownership(&mut ownership_map, &occurrence, false);

        let entry = &ownership_map[&1];
        assert_close(entry.1, 7.95); // consumed
        assert_eq!(entry.3.len(), 1);
    }

    // ─── Avoirs-style regression fixture ─────────────────────────────────────
    // Expected values generated by an independent simulation of the documented
    // conventions (docs/PHASE1_ACCOUNTS.md §5): DAYS360 proration, calendar
    // boundaries, 4-decimal rounding, compounding, strictly-before balance
    // basis. Mirrors the three account archetypes of the reference workbook.

    fn weekly_deposits(
        occurrences: &mut Vec<PaymentOccurrence>,
        pool_id: i64,
        first: &str,
        amount: f64,
        target: NaiveDate,
    ) {
        let mut d = parse_date(first).unwrap();
        let mut id = 1000;
        while d <= target {
    ...
    fn signed_interest(occurrences: &[PaymentOccurrence]) -> Vec<(String, f64)> {
        occurrences
            .iter()
            .map(|o| {
                let signed = if o.receiver_account_id.is_some() {
                    o.amount
                } else {
                    -o.amount
                };
                (o.occurrence_date.clone(), signed)
            })
            .collect()
    }

    #[test]
    fn test_avoirs_fixture_reer_annual_with_weekly_deposits() {
        // Opening 156,691.62 + weekly 740 deposits, 7.61% credited annually.
        // First credit prorated by DAYS360(2026-07-05, 2027-01-01)/360 = 176/360.
        let pool_id = 100;
        let target = NaiveDate::from_ymd_opt(2028, 1, 15).unwrap();
        let mut occurrences = vec![make_occurrence(
            1,
            "2026-07-05",
            156691.62,
            Some(1),
        ...
        weekly_deposits(&mut occurrences, pool_id, "2026-07-10", 740.0, target);
        let rules = vec![make_rule(1, "2026-07-05", None, 0.0761, "annual")];

        let interest =
            generate_interest_occurrences(pool_id, &rules, &occurrences, &HashMap::new(), target);

        assert_eq!(
            signed_interest(&interest),
            vec![
                ("2027-01-01".to_string(), 6517.9069),
                ("2028-01-01".to_string(), 16812.737),
            ]
        );

        // Final balance: deposits + interest must sum to the simulated value
        let deposits: f64 = occurrences.iter().map(|o| o.amount).sum();
        let credited: f64 = signed_interest(&interest).iter().map(|(_, a)| a).sum();
        // 237,742.2639 at the last tick + two weekly deposits before the target
        assert_close(round4(deposits + credited), 239222.2639);
    }

    #[test]
    fn test_avoirs_fixture_mortgage_semiannual_with_renewal() {
        // -428,522.78 opening, weekly 413.41 payments, 1.84% semiannual until
        // the 2027-05-23 renewal, then 3.9375% (prime × 75%).
        let pool_id = 200;
        let target = NaiveDate::from_ymd_opt(2028, 1, 15).unwrap();
        let mut occurrences = vec![make_occurrence(
            1,
        ...
        weekly_deposits(&mut occurrences, pool_id, "2026-07-09", 413.41, target);
        let rules = vec![
            make_rule(2, "2026-07-05", Some("2027-05-23"), 0.0184, "semiannual"),
            make_rule(3, "2027-05-23", None, 0.039375, "semiannual"),
        ];
        ...
        assert_eq!(
            signed_interest(&interest),
            vec![
                ("2026-12-01".to_string(), -3862.5388), // old rate
                ("2027-06-01".to_string(), -8130.0525), // first boundary after renewal
                ("2027-12-01".to_string(), -8078.4987),
            ]
        );
        // Charges are emitted with the pool as payer
        assert!(interest.iter().all(|o| o.payer_id == Some(pool_id)));

        let flows: f64 = 413.41 * (occurrences.len() - 1) as f64 - 428522.78;
        let charged: f64 = signed_interest(&interest).iter().map(|(_, a)| a).sum();
        // -418,414.94 at the last tick + seven weekly payments before the target
        assert_close(round4(flows + charged), -415521.07);
    }

    #[test]
    fn test_avoirs_fixture_checking_credit_line_with_fee() {
        // Credit-line checking: 5.25% only when negative, 7.95 monthly fee
        // below a 5000 balance. Walks through all four regimes: above
        // threshold (nothing), below threshold (fee only), negative
        // (interest + fee), and back above threshold (nothing).
        let pool_id = 300;
        let target = NaiveDate::from_ymd_opt(2027, 2, 15).unwrap();
        let occurrences = vec![
            make_occurrence(1, "2026-07-05", 6000.0, Some(1), Some(pool_id)),
            make_occurrence(2, "2026-08-15", 1500.0, Some(pool_id), Some(1)),
            make_occurrence(3, "2026-10-20", 6000.0, Some(pool_id), Some(1)),
            make_occurrence(4, "2026-12-10", 8000.0, Some(1), Some(pool_id)),
        ];
        let mut rule = make_rule(4, "2026-07-05", None, 0.0525, "monthly");
        rule.only_when_negative = true;
        rule.monthly_fee = Some(7.95);
        rule.fee_threshold = Some(5000.0);
        ...
        assert_eq!(
            signed_interest(&interest),
            vec![
                ("2026-09-01".to_string(), -7.95),    // below threshold: fee only
                ("2026-10-01".to_string(), -7.95),    // still below threshold
                ("2026-11-01".to_string(), -14.5821), // negative: interest + fee
                ("2026-12-01".to_string(), -14.6459),
                // Jan/Feb 2027: back above threshold and positive → nothing
            ]
        );

        let flows: f64 = 6000.0 - 1500.0 - 6000.0 + 8000.0;
        let charged: f64 = signed_interest(&interest).iter().map(|(_, a)| a).sum();
        assert_close(round4(flows + charged), 6454.872);
    }
}
