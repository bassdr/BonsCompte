...
export interface PaymentOccurrence {
  ...
  affects_payer_expectation: boolean;
  affects_receiver_expectation: boolean;
  // Synthetic occurrence generated by an interest rule (payment_id = -rule_id).
  // Hidden from transaction lists; shown in totals breakdowns and charts.
  is_interest: boolean;
}
...
// Interest rules (pool accounts only) — Phase 1, docs/PHASE1_ACCOUNTS.md
export interface AccountInterestRule {
  id: number;
  participant_id: number;
  start_date: string;
  end_date: string | null;
  annual_rate: number; // e.g. 0.05 for 5%
  period: 'monthly' | 'semiannual' | 'annual';
  only_when_negative: boolean;
  monthly_fee: number | null;
  fee_threshold: number | null;
  created_at: string;
}

export interface InterestRuleInput {
  start_date: string;
  end_date?: string | null;
  annual_rate: number;
  period: 'monthly' | 'semiannual' | 'annual';
...
export const getInterestRules = (
  projectId: number,
  participantId: number
): Promise<AccountInterestRule[]> =>
  authFetch(`/projects/${projectId}/participants/${participantId}/interest-rules`);

export const createInterestRule = (
  projectId: number,
  participantId: number,
  data: InterestRuleInput
): Promise<AccountInterestRule> =>
  authFetch(`/projects/${projectId}/participants/${participantId}/interest-rules`, {
    method: 'POST',
    body: JSON.stringify(data)
...
export const updateInterestRule = (
  projectId: number,
  participantId: number,
  ruleId: number,
  data: Partial<InterestRuleInput>
): Promise<AccountInterestRule> =>
  authFetch(`/projects/${projectId}/participants/${participantId}/interest-rules/${ruleId}`, {
    method: 'PUT',
    body: JSON.stringify(data)
  });

export const deleteInterestRule = (
  projectId: number,
  participantId: number,
  ruleId: number
