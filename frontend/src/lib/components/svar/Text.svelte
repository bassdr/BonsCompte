<script lang="ts">
  ...
  function getInputId(id: string | number | undefined): string {
    const register = getContext<(() => string) | undefined>('wx-input-id');
    return String(id || (register && register()) || uid());
  }
  ...
  const oninput = () => {
    if (onchange) onchange({ value, input: true });
  };
  const change = () => {
    if (onchange) onchange({ value });
  };

  function clearValue(ev: MouseEvent) {
    ev.stopPropagation();
    value = '';
    if (onchange) onchange({ value });
  }
...
>
  ...
  {#if clear && !disabled && value}
    ...
    {#if hasLeftIcon}
      <span class="wx-icon"
        >{#if Icon}<Icon size={20} color="currentColor" />{/if}</span
      >
    {/if}
  {:else if icon}
    <span class="wx-icon"
