    ...
    <div class="quarantine-card">
        ...
        <p class="description">
            {$_('approvals.pending.description', {
                default: 'Your account requires approval from project members before you can continue. This is a security measure to ensure account integrity after sensitive changes.'
            })}
        </p>
                    ...
                    <div class="approval-card">
                        ...
                        <div class="approval-status">
                            <div class="progress-bar">
                                <div
                                    class="progress-fill"
                                    style="width: {Math.min(100, ((approval.vote_count || 0) / (approval.required_votes || 1)) * 100)}%"
                                ></div>
                            </div>
                        ...
                        {#if approval.votes && approval.votes.length > 0}
                            <div class="votes-list">
                                ...
                                {#each approval.votes as vote}
                                    <div class="vote-item" class:approve={vote.vote === 'approve'} class:reject={vote.vote === 'reject'}>
                        ...
                        {#if !approval.can_self_approve}
                            <div class="info-message">
                                {$_('approvals.solo_project', { default: 'This is a solo project. Only a system administrator can approve.' })}
                            </div>
                        {/if}
                    </div>
                {/each}
            ...
            <div class="info-box">
                <p>
                    {$_('approvals.pending.help', {
                        default: 'Contact your project administrators to request approval. The page will auto-refresh every 10 seconds.'
                    })}
                </p>
            </div>
        {/if}
